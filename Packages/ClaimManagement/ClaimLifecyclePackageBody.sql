CREATE OR REPLACE PACKAGE BODY ClaimLifecyclePackage AS
    ...
    BEGIN
        -- Step 1: Validate claim for processing
        IF NOT IsClaimValidForValidation(p_claim_id) THEN
            RAISE_APPLICATION_ERROR(-20005, 'Claim is not in progress and cannot be processed.');
        END IF;

        -- Step 2: Fetch the current status for logging
        SELECT CLAIM_STATUS
        INTO v_old_status
        FROM CLAIM
        WHERE CLAIM_ID = p_claim_id;

        -- Step 3: Update claim status and comments
        UPDATE CLAIM
